...
function tern_wp_youtube_video($e=true) {
	...
	$o = $getWP->getOption('tern_wp_youtube',$tern_wp_youtube_options);
	$v = get_post_meta($post->ID,'_tern_wp_youtube_video',true);
	$s = '<iframe title="YouTube video player" width="'.$o['dims'][0].'" height="'.$o['dims'][1].'" src="'.tern_wp_youtube_video_link($v,false).'" frameborder="0" allowfullscreen allowTransparency="true" controls="false"></iframe>';
	if($e) {
		echo $s;
	}
	return $s;
}
function tern_wp_youtube_video_meta($e=false) {
	global $post;
...
function tern_wp_youtube_video_link($i,$e=true) {
	...
	$l .= $o['related'] ? '' : '?rel=0&controls=0&autoplay=0&autohide=1&showinfo=0';
	if($e) { 
		echo $l;
	}
	return $l;
...
function tern_wp_youtube_video_watch_link($i,$e=true) {
	$l = 'http://www.youtube.com/?watch?v='.$i.'&controls=0&autoplay=0&autohide=1&showinfo=0';
	if($e) { echo $l; }
	return $l;
}
			...
			<li class="tern_wp_post post<?php echo $c?>">
				...
				<div class="tern_wp_post_entry">
					<div class="tern_wp_post_image">
						<a href="<?php the_permalink() ?>"><?php tern_wp_youtube_image(); ?></a>
					</div>
